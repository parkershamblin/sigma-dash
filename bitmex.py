...
class TradingChart:
    def __init__(self, symbol):
        ...
        self.fig['data'].append( dict( x=df.index, y=df.volume,                         
        ...
        self.fig['data'].append( dict( x=df.index, y=bb_upper, type='scatter', yaxis='y2', 
                                line = dict( width = 1 ),
                                marker=dict(color='#ccc'), hoverinfo='none', 
                                legendgroup='Bollinger Bands', name='Bollinger Bands') )

        self.fig['data'].append( dict( x=df.index, y=bb_lower, type='scatter', yaxis='y2',
                                line = dict( width = 1 ),
                                marker=dict(color='#ccc'), hoverinfo='none',
...
def market_depth(symbol):
    ...
    ws = BitMEXWebsocket(endpoint="https://testnet.bitmex.com/api/v1", symbol=symbol, api_key="9AyrVqTa4HROjUq3FZfIK-p_", api_secret="EInjEO0WPdasTM4ne7RHN5Q8aBQ-O6HwhFnY3olm7_LKvSNa")
